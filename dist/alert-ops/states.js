...
import { formatAlertOpsTemplate } from './types';
export function resolveAlertOpsView(state) {
    switch (state) {
        case 'loading':
 ...
 * "no results".
 */
export function hasActiveAlertOpsFilters(filters) {
    if (!filters)
        return false;
    return Boolean((filters.states && filters.states.length > 0) ||
        (filters.severities && filters.severities.length > 0) ||
        (filters.service && filters.service.trim() !== '') ||
        (filters.text && filters.text.trim() !== ''));
}
export function resolveAlertOpsCollection(alertCount, filtersActive) {
    if (alertCount > 0)
        return 'rows';
...
export function alertStatusBadgeColor(status) {
    switch (status) {
        case 'firing':
            return 'red';
        case 'resolved':
...
export function providerStateBadgeColor(state) {
    switch (state) {
        case 'ok':
            return 'green';
        case 'loading':
...
export function silenceStateBadgeColor(state) {
...
// ---------- Localized labels ----------
export function providerStateLabel(state, copy) {
    switch (state) {
        case 'ok':
...
export function alertStatusLabel(status, copy) {
    switch (status) {
        case 'firing':
...
export function alertSeverityLabel(severity, copy) {
    switch (severity.trim().toLowerCase()) {
        case 'critical':
...
export function silenceStateLabel(state, copy) {
    switch (state.trim().toLowerCase()) {
        case 'active':
 ...
 */
export function formatAlertOpsTimestamp(iso) {
...
export function formatAlertOpsDuration(totalSeconds, copy) {
...
export function formatAlertOpsRelativeTime(iso, now, copy) {
    if (!iso)
        return copy.lastRefreshNever;
    ...
    if (elapsedMs < 60_000)
        return copy.refreshedJustNow;
    const minutes = Math.floor(elapsedMs / 60_000);
    if (minutes < 60)
        return formatAlertOpsTemplate(copy.refreshedMinutesAgo, { minutes });
    const hours = Math.floor(minutes / 60);
    if (hours < 24)
        return formatAlertOpsTemplate(copy.refreshedHoursAgo, { hours });
...
export function shortAlertFingerprint(fingerprint) {
