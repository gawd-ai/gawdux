...
function readBooleanRouterOption(element, name) {
    let current = element;
...
function eventRouterElement(event) {
...
function captureRouterOptions(event) {
...
export function useGuardedGoto() {
    const controller = getContext(DISCARD_NAVIGATION_CONTEXT);
    return controller ? controller.navigate.bind(controller) : goto;
}
export function navigationTargets(navigation, targetHref) {
    if (!targetHref || !navigation.to)
        return false;
...
export class DiscardNavigationController {
    ...
    requestGuardedAction(owner, request) {
        if (!this.#hasBlockers())
            return false;
        if (this.request || this.surfacePending)
            return true;
    ...
    async #continueHostedAction(requestId, continueAction) {
        if (this.#hostedAction?.request.id !== requestId)
            return;
    ...
    clearAction(owner) {
        if (this.surfacePending || this.#hostedAction?.owner !== owner)
            return;
    ...
    navigate(url, options) {
        ...
        if (this.#pending || this.#hostedAction)
    ...
    intercept(navigation) {
        if (this.#consumeApproval(navigation))
            return;
        if (!this.#hasBlockers(navigation))
            return;
        ...
        if (!navigation.to)
            return;
        const id = this.#nextRequestId++;
        const routerOptions = captureRouterOptions('event' in navigation ? navigation.event : undefined);
        if (routerOptions.replaceState === undefined &&
    ...
    #activeGuards(navigation) {
        return [...this.#guards.values()]
            .map((guard) => {
            const allowed = navigation !== undefined &&
    ...
    #refreshRequest() {
        ...
        const message = dirty.length === 1 && dirty[0]?.message
            ? dirty[0].message
            : dirty.length > 1
    ...
    #consumeApproval(navigation) {
        const approval = this.#approval;
        if (!approval)
            return false;
        if (approval.kind === 'goto' && this.#activeGotoApproval !== approval)
            return false;
        const matches = approval.kind === 'leave'
        ...
        const blockersChanged = activeGuards.length !== approval.guardStates.size ||
            activeGuards.some((guard) => approval.guardStates.get(guard.id) !== `${Number(guard.dirty)}:${Number(guard.busy)}`);
        if (blockersChanged) {
            this.#clearApproval();
            this.#refreshRequest();
    ...
    async #retry(requestId) {
        ...
        const guardStates = new Map(this.#activeGuards().map((guard) => [
            guard.id,
        ...
        if (pending.type === 'popstate' && pending.delta != null) {
            ...
            if (currentGuards.length !== guardStates.size ||
                currentGuards.some((guard) => guardStates.get(guard.id) !== `${Number(guard.dirty)}:${Number(guard.busy)}`)) {
        ...
        try {
            ...
            if (this.#activeGotoApproval === approval)
                this.#activeGotoApproval = null;
            if (this.#approval === approval)
                this.#clearApproval();
        }
    ...
    async #waitForRollback(fromHref) {
        if (this.#runtime.currentHref() === fromHref)
            return;
        await new Promise((resolve, reject) => {
